...
UPDATE ORDERS SET ORDER_AMT = 141300
WHERE ORDER_NUM = '202501090002';
...
DELETE FROM 테이블명
[WHERE 조건]
. WHERE 절이 생략되면 모든 행을 삭제
. ROLLBACK 의 대상
. 삭제 대상의 값이 자식테이블에서 사용되고 있으면 삭제되지 않음
  재귀 삭제는 CASCADE옵션이 ENABLE 된 경우에는 자식부터 지우고 부모테이블의
  행을 지움
  
사용예) HR계정의 JOB_HISTORY테이블의 자료 중 사원번호(EMPLOYEE_ID)가 101 인 사원정보를 삭제하시오.
;
